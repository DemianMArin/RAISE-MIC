...
AUDIO_FS = 16000  # playback + file sample rate
...
class RAISEApp(QWidget):
    ...
    def handle_notify(self, handle, data):
        ...
        seq = data[0]
        mv = int.from_bytes(data[1:], byteorder='little', signed=True)
        self.analog_data.append(mv)
        self.audio_buffer.append(mv)
        ...
        if dt >= 1.0:
            byte_count = len(self.sequence_numbers) * 5  # 1 byte seq + 4 byte voltage
            bitrate = byte_count * 8 / dt
            self.bitrate_values.append(bitrate)

            if self.last_seq is not None:
                expected = (self.last_seq + len(self.sequence_numbers)) % 256
